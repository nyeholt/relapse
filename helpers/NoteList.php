	...
	public function NoteList($notes, $createurl, $userlist = array(), $watchers=array())
	{
	    ...
	    <h3>Notes</h3>
	        ...
	        <div class="note">
	            ...
	            <a style="float: right; " href="#" onclick="if (confirm('Do you want to delete this note?')) { location.href='<?php echo build_url('note', 'delete', array('id'=>$note->id))?>'; } return false;"><img src="<?php echo resource('images/delete.png')?>" /></a>
	            <?php endif; ?>
	                <h4><?php $this->view->o($note->title)?></h4>
	                <span class="note-by">By <?php $this->view->o($note->userid.' on '.$note->created)?></span>
	            </div>
	        ...
	        <div class="note">
	        	<div class="padded">
		        <form id="issue-add-note-form" method="post" action="<?php echo $createurl?>">
		            ...
		            <p class="clear">
		                ...
		                <a title="Unsubscribe" style="display: <?php echo $deleteStyle?>;" id="delete-watch" href="#" onclick="$.get('<?php echo build_url('note', 'deletewatch')?>', {id:'<?php echo $this->view->model->id?>', type:'<?php echo get_class($this->view->model)?>'}, function() {$('#delete-watch').hide();$('#add-watch').show(); }); return false; "><img src="<?php echo resource('images/thumb_down.png')?>"/></a>
			        	<a title="Subscribe" style="display: <?php echo $addStyle?>;" id="add-watch" href="#" onclick="$.get('<?php echo build_url('note', 'addwatch')?>', {id:'<?php echo $this->view->model->id?>', type:'<?php echo get_class($this->view->model)?>'}, function() {$('#add-watch').hide();$('#delete-watch').show(); }); return false; "><img src="<?php echo resource('images/thumb_up.png')?>"/></a>
		            </p>
		            <p>
	        ...
	        <div>
				<form action="<?php echo build_url('note', 'setwatchers')?>" method="post">
					<input type="hidden" value="<?php echo get_class($this->view->model)?>" name="attachedtotype"/>
		            <input type="hidden" value="<?php echo $this->view->model->id?>" name="attachedtoid"/>
					<p>
