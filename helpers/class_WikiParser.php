...
class WikiParser {
	...
	function handle_list($matches,$close=false) {
		...
		while ($this->list_level!=$newlevel) {
			...
			$listtype = ifset($listtypes, $listchar);
	...
	function emphasize($amount) {
		...
		if ( (!ifset($this->emphasis, $amount)) && (ifset($this->emphasis, $amount-1)) ) {
			$amount--;
			$output = "'";
		}
...
//			'link'=>'(\[\[((.*?)\:)?(.*?)(\|(.*?))?\]\]([a-z]+)?)',
		...
		if (($this->list_level>0) && !ifset($called, 'list')) $line = $this->handle_list(false,true) . $line;
		if ($this->deflist && !$called['definitionlist']) $line = $this->handle_definitionlist(false,true) . $line;
		if ($this->preformat && !ifset($called, 'preformat')) $line = $this->handle_preformat(false,true) . $line;
		
		// suppress linebreaks for the next line if we just displayed one; otherwise re-enable them
		if ($isline) $this->suppress_linebreaks = (ifset($called, 'newline') || ifset($called, 'sections'));
