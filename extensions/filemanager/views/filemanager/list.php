...
<?php if (mb_strlen($this->parentPath)): ?>
	<li>
		<a href="<?php echo build_url('file', 'index', array('folder' => base64_encode($this->parentPath)))?>">Parent Directory</a>
	</li>
<?php endif; ?>
<?php foreach ($this->files as $file):?>
	<li>
	<?php if (is_string($file)): ?> 
		<a href="<?php echo build_url('file', 'index', array('folder' => base64_encode($this->base.$file)))?>"><?php $this->o($file)?></a>

	<?php else: ?> 
		<a class="action-icon" title="Edit file" href="<?php echo build_url('file', 'edit', array('id'=>$file->id, 'returnurl'=>base64_encode('file/index/folder/'.base64_encode($file->path)), 'parent'=>base64_encode($file->path)))?>">
			<img src="<?php echo resource('images/pencil.png')?>" />
		</a> 

		<?php if ($this->u()->hasRole(User::ROLE_USER)): ?>
		<a class="action-icon" title="Delete file" href="<?php echo build_url('file', 'delete', array('id'=>$file->id, 'returnurl'=>base64_encode('file/index/folder/'.base64_encode($file->path))))?>">
...
<form method="post" action="<?php echo build_url('file', 'upload');?>" enctype="multipart/form-data">
	...
	value="<?php echo base64_encode('file/index/folder/'.base64_encode($this->base))?>" />
	<input type="hidden" value="<?php echo base64_encode($this->base)?>" name="parent" />
