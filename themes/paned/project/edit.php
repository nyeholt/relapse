...
<form method="post" action="<?php echo build_url('project', 'save');?>" class="ajaxForm data-form">
...
<?php endif; ?>
	<fieldset>
		...
		<?php $this->selectList('Client', 'clientid', $this->clients, $this->client ? $this->client->id : 0) ?>
	...
	<p class="clear">
		...
		<?php if ($this->model->id && !$this->model->ismilestone): ?>
			<input type="button" class="abutton" onclick="$.post('<?php echo build_url('project', 'recalculate', array('id' => $this->model->id))?>')" value="Calculate Estimates" />
		<?php endif; ?>

		<?php if ($this->viaajax): ?>
			<input type="button" class="abutton" onclick="Relapse.closeDialog('projectdialog', this)" value="Close" />
		<?php else: ?>
